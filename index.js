...
// Set the current year as max year
...
const validateForm = (e) => {
  ...
  // Set the range for days
  const daysInMonth = new Date(yyyy, mm, 0).getDate();
  inputDay.setAttribute("max", daysInMonth);

  // Check if input years is not from the future
  ...
  if (inputYear.validity.valueMissing) {
    inputYear.setCustomValidity(errorMessage.required);
  }
  else if (inputYear.validity.rangeUnderflow) {
    inputYear.setCustomValidity(errorMessage.invalid("year"));
  }
  else if (inputYear.rangeOverflow || isFuture) {
    inputYear.setCustomValidity(errorMessage.range);
  }
  else {
  ...
  if (inputMonth.validity.valueMissing) {
    inputMonth.setCustomValidity(errorMessage.required);
  }
  else if (
    inputMonth.validity.rangeUnderflow ||
    inputMonth.validity.rangeOverflow
  ) {
    inputMonth.setCustomValidity(errorMessage.invalid("month"));
  }
  else {
  ...
  if (inputDay.validity.valueMissing) {
    inputDay.setCustomValidity(errorMessage.required);
  }
  else if (
    ...
    inputDay.setCustomValidity(errorMessage.invalid("date"));
  }
  else if (
    inputDay.validity.rangeUnderflow ||
    inputDay.validity.rangeOverflow
  ) {
    inputDay.setCustomValidity(errorMessage.invalid("day"));
  }
  else {
  ...
  // If all check out calculate the age else reset result 
